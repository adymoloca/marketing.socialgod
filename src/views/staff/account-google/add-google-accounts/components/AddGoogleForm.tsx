...
import MultipleGoogleAccounts from './MultipleGoogleAccounts';
import FormikGoogleAccount from './FormikGoogleAccounts';
...
const AddGoogleForm: FC = () => {
	...
	const { addGoogleAccountsAction, loading, setMessage } = useGoogleAccounts();
	...
	const handleAccounts = (number: number): void => {
		const valueReduced = number >= 30 ? 30 : number;
		const rightValue = number < 1 ? 1 : valueReduced;
	...
	const handleSuccess = (): void => {
		...
		setMessage('');
		navigate('/staff/accounts-google');
	};

	const handleSubmit = (values: SentObject[]): void => {
		const formatedObject = { accounts: values };
		addGoogleAccountsAction(formatedObject, handleSuccess);
	};
	...
	return (
		<Grid container sx={{ justifyContent: 'start' }}>
			...
			{accountType === 'custom' ? (
				...
				<MultipleGoogleAccounts setData={setData} />
			)}
			<ButtonSG
