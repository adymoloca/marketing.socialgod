...
const MultipleGoogleAccounts: FC<IParams> = (props) => {
	...
	const handleBlurFormat = useCallback(
		(values: IInitial) => {
			...
			for (let i = 0; i < arr?.length; i++) {
				...
				sentData.push({ email: account[0], password: account[1] });
			}
						...
						<Form noValidate onSubmit={handleSubmit} style={{ width: '100%' }}>
							...
							<FormControl fullWidth error={Boolean(touched.accounts && errors.accounts)}>
								...
								<Typography sx={{ fontSize: '14px', color: '#696969', textAlign: 'start', ml: 1 }}>
									{capitalize('You need to provide a list of accounts to add ( email: password ) *')}
								</Typography>
...
export default MultipleGoogleAccounts;
